...
I3=rgb2gray(I2);
I3=im2double(I3);%doubleprecisiion
I3=im2bw(I3);                       
...
N=k-1;
%the original x&y co-ordinate center of mass of the image
oub=sum(C(1,:))/N;  
ovb=sum(C(2,:))/N; 
 % Rotate moving the signature to the origin
for i=1:N
    u(i)=u(i)-oub+1;
    v(i)=v(i)-ovb+1;
end
...
%the image the signature might go out of the boundry (128x128) therefore we have to move the signature curve
mx=0;
my=0;
if (min(u)<0)
    mx=-min(u);
